...
function calculateLvl() {
	...
	let B =
		(pokemons_caught[currentPokemonIndex].base_experience * (enemy_hp / 10)) /
		5;

	//floor( floor(√(A) * (A * A)) * B / floor(√(C) * (C * C))) + 1
	let exp =
		Math.floor(
			(Math.floor(Math.sqrt(A) * (A * A)) * B) /
				Math.floor(Math.sqrt(C) * (C * C))
		) + 1;
	console.log(exp);
	return exp;
}

function fixMoney(decimal) {
	if (myMoney.toString().includes('.')) {
		myMoney = myMoney.toFixed(decimal);
