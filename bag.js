...
function showBag(){
  ...
  for(let i = 0; i < myBag.length; i++){
    bagContents += "<li><button onclick='catchPokemon()'><img src='"+myBag[i].image+"'/><h3>x"+myBag[i].quantity+"</h3></button></li>";
  }
  bagContents += "</ol>";
...
function showShop(){
  document.getElementById("shop").innerHTML = "<h3>Shop</h3><form><div><label for='pokeball-number'></label><input type='number' id='pokeball-number'/></div><div>1 pokeball = "+pokeballPrice+" Bucks</div><div><input type='button' onclick='shopPokeball()' value='shop'/></div></form>";
}
